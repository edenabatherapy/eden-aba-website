...
export default function Ados2ScheduleForm({ t }) {
  ...
  const handleSubmit = (event) => {
    ...
    // TODO: Connect to Eden ABA Therapy ADOS-2 appointment request API endpoint.
    if (typeof window !== "undefined") {
      localStorage.setItem("eden-ados2-appointment-placeholder", JSON.stringify(payload));
    }
    setSuccess(true);
  };
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm font-black text-slate-700">
          {f.fields.postalCode}
          <input required className={fieldClass} value={form.postalCode} onChange={(e) => updateForm("postalCode", e.target.value)} />
        </label>
        <label className="grid gap-2 text-sm font-black text-slate-700">
        ...
        <label className="grid gap-2 text-sm font-black text-slate-700 md:col-span-2">
          {f.fields.message}
          <textarea rows={4} className={fieldClass} value={form.message} onChange={(e) => updateForm("message", e.target.value)} />
        </label>
      </div>
