    ...
    <section className="px-4 py-20 lg:px-8 dark:bg-slate-950" aria-labelledby="eden-difference-heading">
      <div className="mx-auto max-w-7xl">
        ...
        <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.08 }} className="mt-10">
          <GlassPanel className="mx-auto max-w-4xl text-center">
            <p className="text-base leading-8 text-slate-700 dark:text-slate-300 sm:text-lg">{content.intro}</p>
          </GlassPanel>
        </motion.div>
        ...
        >
          {content.cards.map((card, index) => {
            const Icon = DIFFERENCE_ICONS[index] || Target;
            return (
