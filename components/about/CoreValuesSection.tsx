              ...
              <motion.div key={value.title} variants={staggerItem}>
                <Card
                  className={`h-full transition duration-300 dark:border-slate-700 dark:bg-slate-900 ${
                    isExpanded
                      ? "-translate-y-1 border-emerald-300 shadow-xl shadow-emerald-900/10"
                ...
                >
                  ...
                  <CardContent>
                    ...
                    <div
                      ...
                      id={`value-details-${value.title.replace(/\s+/g, "-").toLowerCase()}`}
                    >
                      <p className="text-sm leading-7 text-slate-600 dark:text-slate-300">{value.fullDescription}</p>
                    </div>
                    <button
                      type="button"
                      className="mt-4 text-sm font-bold text-emerald-700 underline-offset-4 transition hover:text-emerald-900 hover:underline dark:text-emerald-300 dark:hover:text-emerald-200"
                      aria-expanded={isExpanded}
                      aria-controls={`value-details-${value.title.replace(/\s+/g, "-").toLowerCase()}`}
