    ...
    <section className="bg-slate-50 px-4 py-20 dark:bg-slate-900/50 lg:px-8" aria-labelledby="holistic-development-heading">
      <div className="mx-auto max-w-7xl">
        <motion.div {...fadeUp} className="mx-auto max-w-3xl text-center">
          ...
          <h2
            id="holistic-development-heading"
        ...
        >
          {content.areas.map((area, index) => {
            const Icon = AREA_ICONS[index] || MessageCircle;
            return (
              <motion.div key={area.title} variants={staggerItem}>
                <Card className="h-full transition duration-300 hover:-translate-y-1 hover:border-emerald-300 hover:shadow-xl dark:border-slate-700 dark:bg-slate-900">
                  <CardHeader>
                    ...
                    <CardTitle className="mt-4 text-xl">{area.title}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-base leading-7 text-slate-600 dark:text-slate-300">{area.description}</p>
                  </CardContent>
                </Card>
              </motion.div>
            );
