...
import {
  RBT_FAQ_ITEMS,
  RBT_GROWTH_LADDER_DETAILED,
  RBT_INTERVIEW_TIPS,
  RBT_RESPONSIBILITIES,
...
export default function RbtCareersTabs() {
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent, index: number) => {
      ...
      if (next !== index) {
        ...
        document.getElementById(`rbt-tab-${TABS[next].id}`)?.focus();
      }
    },
    [],
  );
  ...
  if (isMobileAccordion) {
    return (
      <section aria-labelledby="rbt-tabs-heading">
        <h2 id="rbt-tabs-heading" className="text-2xl font-extrabold text-slate-900 sm:text-3xl">
          Explore the RBT role
        </h2>
        <p className="mt-3 text-base leading-8 text-slate-600">
          Review responsibilities, skills, growth options, interview guidance, and common questions.
        </p>
        <div className="mt-8 space-y-3">
          {TABS.map((tab) => {
            ...
            return (
              <div key={tab.id} className="overflow-hidden rounded-[1.25rem] border border-emerald-100 bg-white shadow-sm">
                <button
                  type="button"
                  id={`rbt-tab-${tab.id}`}
                  aria-expanded={open}
                  aria-controls={`rbt-panel-${tab.id}`}
                ...
                {open && (
                  <div id={`rbt-panel-${tab.id}`} className="border-t border-emerald-50 px-4 pb-5 pt-2">
  ...
  return (
    <section aria-labelledby="rbt-tabs-heading">
      ...
      <p className="mt-3 max-w-3xl text-base leading-8 text-slate-600">
        Review responsibilities, skills, growth options, interview guidance, and common questions in one place.
      </p>

      <div
        role="tablist"
        aria-label="RBT role details"
        className="mt-8 flex flex-wrap gap-2 border-b border-emerald-100 pb-1"
      >
        {TABS.map((tab, index) => {
      ...
      <div className="mt-6 min-h-[280px] overflow-hidden rounded-[1.5rem] border border-emerald-100 bg-white p-6 shadow-sm sm:p-8">
        <AnimatePresence mode="wait">
          <motion.div
            key={activeTab}
            role="tabpanel"
            id={`rbt-panel-${activeTab}`}
            aria-labelledby={`rbt-tab-${activeTab}`}
